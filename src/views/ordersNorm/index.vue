<template>
    <div class="warpper">
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column fixed prop="normType" label="规格简称"> </el-table-column>
            <el-table-column
  ...
  <script>
  import {selectByNormNo,selectAllByNormNoGraph,selectMonthByNorm} from '@/api/ordersInteractions'
  ...
  export default {
    ...
    mounted() {
      this.selectByNormNo();
      this.selectAllByNormNoGraph()
      this.selectMonthByNorm()
    ...
    methods: {
      ...
      async selectMonthByNorm(){
        const params={
          year:this.form.year,
          month:this.form.month,
        }
        const res = await selectMonthByNorm(params)
        if(res.code === '0'){
          ...
          this.barTask = res.data.map((v)=>{
            ...
            return {
              ...
              type:"bar",
              barWidth: '30%',
              data:[v.allTask,v.norm],
            }
          })
          this.barSubmit = res.data.map((v)=>{
            submitArr.push(v.norm)
            return {
              name:v.norm,
              type:"bar",
              barWidth: '30%',
              data:[v.allSubmit,v.norm],
            }
          })
          this.$nextTick(()=>{
      ...
      async selectAllByNormNoGraph(){
        const res = await selectAllByNormNoGraph(this.form)
        if(res.code==='0')
        {
          this.graphTask = res.data.map((v)=>{return {value:v.allTask,name:v.normType}})
          this.graphSubmit = res.data.map((v)=>{return {value:v.allSubmit,name:v.normType}})
          this.$nextTick(()=>{
            ...
            let option = {
                ...
                series: [
                  {
                    center: ['64%', '45%'],           // 饼图的中心（圆心）坐标，数组的第一项是横坐标，第二项是纵坐标。[ default: ['50%', '50%'] ]
				            // radius: ['30%', '40%'],
                    // name: 'Access From',
                    type: 'pie',
                    radius: '50%',
      ...
      async selectByNormNo() {
        const params = {
          ...this.form,
        };
        const res = await selectByNormNo(params);
        if (res.code === "0") {
