    ...
    <div class="warpper">
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column prop="mesInOutWeight" label="入库净重">
          </el-table-column>
          <el-table-column prop="mesInOutRatio" label="入库净重环比%">
          </el-table-column>
          <el-table-column
            prop="mesInOutAverage"
            label="入库净重日均值"
      ...
      <div style="margin-top: 10px;">
        ...
        <el-card class="box-card" style="margin-top: 10px;">
          ...
          <div v-show="weightYear.length!==0" id="mainWeight" class="line" style="height: 300px; width: 100%"></div>
          <el-empty v-show="weightYear.length==0"  description="暂无数据" style="height: 320px; width: 100%"></el-empty>
        </el-card>
      </div>
...
import {
  ...
  StatisticsReports,
  exportInDetailWithYear,
  StatisticsReportsRatioYear,
  StatisticsReportsWeightYear} from '@/api/storeIn'
...
export default {
    ...
    methods:{
      async hadelExcel(){
        const res = await exportInDetailWithYear(this.query.time)
        const blob = new Blob([res], { type: "application/vnd.ms-excel" });
          const fileName = "成品入库（年）汇总表.xlsx";
      ...
      getSummaries(param) {
        ...
        columns.forEach((column, index) => {
          ...
          const values = data.map(item => {
            if(column.property=='mesInOutWeight'){
            ...
            }else if(column.property=='mesInOutAverage'){
              return Number(item[column.property])
            }else if(column.property=='reWorkWeight'){
              return Number(item[column.property])
      ...
      async StatisticsReports(){
        const res = await StatisticsReports()
        if(res.code==='0'){
          this.obj=res.data[0]
        }
      },
      ...
      async InDetailWithYear(){
          const params={
            ...
            // mesCustomer:this.query.mesCustomer?this.query.mesCustomer+'':this.query.mesCustomer,
            // mesNormName:this.query.mesNormName?this.query.mesNormName+'':this.query.mesNormName,
            // meterLength:this.query.meterLength?this.query.meterLength:meterLength,
            // wheelsType:this.query.wheelsType?this.query.wheelsType:wheelsType
          }
          const res = await InDetailWithYear(params)
          if(res.code==='0'){
