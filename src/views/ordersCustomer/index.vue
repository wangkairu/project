<template>
    <div class="warpper">
      ...
      <div class="table">
        ...
        >
          ...
          <el-table-column fixed prop="customerName" label="客户简称"> </el-table-column>
         <el-table-column
          ...
          >
            <template slot-scope="scope">
              <span v-show="v.value==='taskNumber'&&!scope.row.flag&&!v.check">{{ scope.row.contents[v.index].task}}</span>
              <span v-show="v.value==='submitNumber'">{{ scope.row.contents[v.index].submit}}</span>
              <span v-show="v.value==='finishRatio'">{{ scope.row.contents[v.index].finishRatio}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="allTaskNumber" label="合计订单量"> </el-table-column>
          <el-table-column prop="allSubmit" label="合计交付量"> </el-table-column>
  ...
  <script>
  import {selectCustomerYearSubmit,selectAllByCustomerNameGraph,selectMonthByCustomer} from '@/api/ordersInteractions'
    ...
    methods: {
      getSummaries(param){
        ...
        columns.forEach((column, index) => {
          ...
          const arr = data.map(item => item.contents); 
          let values  = arr.flat(1)
          let sum =0 
          let total=0
          if(column.label.includes('订单量')){
            values.forEach((v)=>{
              if(index===1&&v.month===1){
                sum+=v.task
                sums[index] = sum
              } else if(index===4&&v.month===2){
                sum+=v.task
                sums[index] = sum
              } else if(index===7&&v.month===3){
                sum+=v.task
                sums[index] = sum
              } else if(index===10&&v.month===4){
                sum+=v.task
                sums[index] = sum
              } else if(index===13&&v.month===5){
                sum+=v.task
                sums[index] = sum
              } else if(index===16&&v.month===6){
                sum+=v.task
                sums[index] = sum
              } else if(index===19&&v.month===7){
                sum+=v.task
                sums[index] = sum
              } else if(index===22&&v.month===8){
                sum+=v.task
                sums[index] = sum
              } else if(index===25&&v.month===9){
                sum+=v.task
                sums[index] = sum
              } else if(index===28&&v.month===10){
                sum+=v.task
                sums[index] = sum
              } else if(index===31&&v.month===11){
                sum+=v.task
                sums[index] = sum
              } else if(index===34&&v.month===12){
                sum+=v.task
                sums[index] = sum
              }
            })
            data.forEach((v)=>{
              if(index===37){
                total+=v.allTaskNumber
                sums[index] = total
              }
            })
          } else if(column.label.includes('交付量')){
            values.forEach((v)=>{
              if(index===2&&v.month===1){
                sum+=v.submit
                sums[index] = sum
              } else if(index===5&&v.month===2){
                sum+=v.submit
                sums[index] = sum
              } else if(index===8&&v.month===3){
                sum+=v.submit
                sums[index] = sum
              } else if(index===11&&v.month===4){
                sum+=v.submit
                sums[index] = sum
              } else if(index===14&&v.month===5){
                sum+=v.submit
                sums[index] = sum
              } else if(index===17&&v.month===6){
                sum+=v.submit
                sums[index] = sum
              } else if(index===20&&v.month===7){
                sum+=v.submit
                sums[index] = sum
              } else if(index===23&&v.month===8){
                sum+=v.submit
                sums[index] = sum
              } else if(index===26&&v.month===9){
                sum+=v.submit
                sums[index] = sum
              } else if(index===29&&v.month===10){
                sum+=v.submit
                sums[index] = sum
              } else if(index===32&&v.month===11){
                sum+=v.submit
                sums[index] = sum
              } else if(index===35&&v.month===12){
                sum+=v.submit
                sums[index] = sum
              }
            })
      ...
      async selectCustomerYearSubmit() {
        const params = {
          ...this.form,
        };
        const res = await selectCustomerYearSubmit(params);
        if (res.code === "0") {
          this.data = res.data.map((v)=>{return {...v,flag:false}})
          let arr = []
          res.data.forEach((v)=>{
            arr=v.contents.map((item,ind)=>{
              return [
