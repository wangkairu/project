    ...
    <div class="coupon">
        ...
        <div class="con">
            ...
            <!-- <van-button size="small" type="info" @click="checkAll">全选</van-button> -->
                <van-button size="small"   type="info" @click="handelCreate">创建返库单</van-button>
                <van-button size="small" style="margin-left: 10px;"  type="info" @click="handelStartDispatch">开始返库</van-button>
                <!-- <van-button size="small"   type="info" @click="handelReturn">开始返库</van-button> -->
                <van-button size="small"  style="margin-left: 10px;" type="info" @click="handelCancel">批量删除</van-button>
            </div>
            ...
            <div class="data" v-if="data&&data.length>0">
                <div class="con-items" v-for="v in data" :key="v.id">
                    ...
                    <div class="detail" v-if="v.folder">
                        ...
                        <li>已完成数量：{{v.completeNum}}</li>
                        <li>合同号{{v.contractNo}}</li>
                        <li>发货箱数：{{v.deliverBoxNum}}</li>
                        <li>发货毛重：{{v.deliverGrossWeight}}</li>
                        <li>发货净重：{{v.deliverNetWeight}}</li>
                        <li>文档输入框1：{{v.documentInputBox1}}</li>
                        <li>文档输入框2：{{v.documentInputBox2}}</li>
                        <li>计划箱数左：{{v.planBoxCountLeft}}</li>
                        <li>计划箱数右：{{v.planBoxCountRight}}</li>
                        <li>每箱减少重量：{{v.reduceWeightPerBox}}</li>
                        <li>生产日期：{{v.produceDate}}</li>
                        <li>wms发货类型：{{v.wmsDeliverTypeEnum}}</li>
                        <li>状态：{{v.statusEnum}}</li>
                        <li>备注：{{v.remark}}</li>
...
import {queryTurnBackList,startTurnBack,deleteTurnBack} from '@/api/return'
    ...
    methods:{
        ...
        async handelCancel(){
            if(this.ids.length==0){
                ...
                const res = await deleteTurnBack(this.ids)
                if(res.data.code==='0')
                {
                    this.queryTurnBackList()
                }
