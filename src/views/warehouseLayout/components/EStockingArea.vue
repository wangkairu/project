          ...
          <el-table-column label="操作" width="120">
            <template slot-scope="scope">
              <!-- <el-button
                ...
                size="mini"
                type="text"
                @click="handelEnable(scope.row)">启用</el-button>
              <el-button
  ...
  export default {
    ...
    //   },
      async basWarehouseVirtually() {
        ...
        if (res.code === '0') {
            let arr = res.data["WAREHOUSE-D-EAST"]
            for(let key in arr){
                this.newArr.push({
                  value:key,
                  label:arr[key],
        ...
        for (let i = 0; i < this.westAreaList.length; i++) {
            ...
            for(let k = 0; k < row.length; k++){
                ...
                for(let j = 0; j < col.length; j++){
                    ...
                    const width = (this.width * col.length)  + (col.length * 20)
                    const height = this.height * row.length + (row.length * 10)
                    if(width*count>container.clientWidth){
                        count = 0;
                        this.row++;
                    }
                    ...
                    if(smallRect.c === 1){
                      this.rect1 = new Konva.Rect({
                        x:width*count+ (this.width * smallRect.y) + (smallRect.y  * 8),
                        y:this.row*(height+15) + this.height * smallRect.x + (smallRect.x * 10),
                        width: 20,
                        height: 15,
                        fill: 'yellow',
                      });
                    }else if(smallRect.c === 0){
                      this.rect1 = new Konva.Rect({
                        x:width*count+ (this.width * smallRect.y) + (smallRect.y  * 8),
                        y:this.row*(height+15) + this.height * smallRect.x + (smallRect.x * 10),
                        width: 20,
                    ...
                    this.rect1.on('mousedown', (e) => {
                        ...
                        this.stationCode = `E&${this.westAreaList[i].text}&${smallRect.x}-${smallRect.y}`
