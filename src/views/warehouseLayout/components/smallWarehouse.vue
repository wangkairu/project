  ...
  export default {
    ...
    handelEndRow(){
      ...
      const batchDelete =  new Konva.Rect({
        x: 43+this.width * col + (col * 8) ,
        y: 54+this.height*this.form.startRow+ (this.form.startRow * 6),
        width: 24,
    ...
    handelStartRow(){
        ...
        .then(async () => {
          const res = await clearLocationWithoutGoods(val)
            if (res.code === '0') {
              ...
              if (res.data == 2) {
                this.rect1 = new Konva.Rect({
                  x:this.container.clientWidth>=1633?308+ this.width * x + (x * 8):45+ this.width * x + (x * 8),
                  y: this.container.clientWidth>=1633?165+ this.height * y + (y * 6):57+ this.height * y + (y * 6),
                  width: 20,
                  height: 15,
                  fill: 'green',
                });
              ...
              this.rect1.on('mousedown', (e) => {
                if (e.evt.button === 0) {
                  ...
                  this.stationCode = `E&B&${this.x}-${this.y}`
      ...
      async getGoodsLocation(){
        ...
        if(res.code==='0'){
          ...
          this.rect1.on('mouseover',(e) => {
            this.x = smallRect.x; this.y = smallRect.y
            this.stationCode = `E&B&${smallRect.x}-${smallRect.y}`
            time=setTimeout(() => {
        ...
        .then(async () => {
          ...
          const res = await clearLocation(params)
            if (res.code === '0') {
              ...
              if (res.data == 2) {
                this.rect1 = new Konva.Rect({
                  x:this.container.clientWidth>=1633?308+ this.width * this.x + (this.x * 8):45+ this.width * this.x + (this.x * 8),
                  y: this.container.clientWidth>=1633?165+ this.height * this.y + (this.y * 6):57+ this.height * this.y + (this.y * 6),
                  width: 20,
                  height: 15,
                  fill: 'green',
                });
      ...
      async occupyrResourceShow(){
        if(this.layer){
          const res = await occupyrResourceShow("WAREHOUSE-D-EAST/B")
          ...
          if(this.luckArr){
              ...
              x: this.container.clientWidth>=1633?306+this.width * this.luckArr[i].col + (this.luckArr[i].col * 8) :43+this.width * this.luckArr[i].col + (this.luckArr[i].col * 8),
              y: this.container.clientWidth>=1633?162+this.height*this.luckArr[i].rowStart+ (this.luckArr[i].rowStart * 6):54+this.height*this.luckArr[i].rowStart+ (this.luckArr[i].rowStart * 6),
              width: 24,
     ...
     async occupyResourceRelease(val){
      ...
      init() {
        ...
        this.wStage.on('mouseout',(e) => {
          this.luky=false
        })
        this.imageObj = new Image();
        this.imageObj.onload = () => {
          this.image = new Konva.Image({
            x: this.container.clientWidth>=1633?300:50,
            y: this.container.clientWidth>=1633?150:50,
            image: this.imageObj,
            width: 1000,
            height: this.container.clientWidth>=1633?560:450,
          });
          ...
          this.image.on('mouseout',(e) => {
            this.luky=false
          })
          for (let i = 0; i < this.westAreaList.length; i++) {
            const row = this.westAreaList[i]
            const rowText = new Konva.Text({
              x:this.container.clientWidth>=1633? 335+ 20 * i + (i * 8):70+ 20 * i + (i * 8),
              y:this.container.clientWidth>=1633?-15+ 20 + (20 * 8):57,
              text: i+1,
              fontSize: 18,
              fontFamily: 'Calibri',
              fill: 'green',
            ...
            for (let j = 0; j < row.length; j++) {
              const colText = new Konva.Text({
                x: this.container.clientWidth>=1633?310:52,
                y:this.container.clientWidth>=1633?187+15*j+(j*6):78+15*j+(j*6),
                text: j+1,
                fontSize: 15,
                fontFamily: 'Calibri',
                fill: 'green',
              ...
              if (smallRect.e === 0) {
              this.rect1 = new Konva.Rect({
                x:this.container.clientWidth>=1633?308+ this.width * smallRect.x + (smallRect.x * 8):45+ this.width * smallRect.x + (smallRect.x * 8),
                y: this.container.clientWidth>=1633?165+ this.height * smallRect.y + (smallRect.y * 6):57+ this.height * smallRect.y + (smallRect.y * 6),
                width: 20,
                height: 15,
                strokeWidth: 1,
                stroke: '#999',
