<template>
    <div class="warpper">
        ...
        <el-card class="box-card" style="flex: 2;margin: 10px 0;">
            ...
            <div id="materialsYear" style="height: 300px;"></div>
        </el-card>
    </div>
</template>
...
import {exportTemplate,importFinishedMaterials,showFinishedMaterials} from '@/api/materials'
export default {
    components:{
        SearchFilter
    },
    ...
    methods:{
        ...
        async showFinishedMaterials(){
            const params={
                ...this.form,
            }
            const res = await showFinishedMaterials(params)
            if(res.code==='0')
            {
                this.data=res.data
                this.mesCount=[]
                ...
                let newArr=[]
                for(let i=1; i<=12;i++){
                    newArr.push(`${this.form.year}-${i}`)
                }
                let materialsYear =  echarts.init(document.getElementById('materialsYear'));
                let materialsYearOptions = {
                    grid:{
                        top:"10%",
                        left:"2%",
                        right:"4%",
                ...
                materialsYear.setOption(materialsYearOptions);
            }
        },
        handelExcelExport(){
