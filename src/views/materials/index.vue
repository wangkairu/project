...
import {exportTemplate,importFinishedMaterials,showFinishedMaterialsMonth} from '@/api/materials'
...
export default {
    ...
    mounted(){
        this.showFinishedMaterialsMonth()
    },
    methods:{
        search(){
            this.showFinishedMaterialsMonth()
        ...
        async showFinishedMaterialsMonth(){
            const params={
                ...this.form,
                year:this.form.month.split('-')[0],
                month:this.form.month.split('-')[1],
            }
            const res = await showFinishedMaterialsMonth(params)
            if(res.code==='0')
            {
                this.data=res.data
                this.mesCount=[]
                ...
                const days = new Date(this.form.month.split('-')[0], this.form.month.split('-')[1], 0).getDate()
                let newArr=[]
                for(let i=1; i<=days;i++){
                    newArr.push(`${this.form.month.split('-')[0]}-${this.form.month.split('-')[1]}-${i}`)
                }
        ...
        handelExcelImport(file){
            const formData = new FormData();
            formData.append("file", file.file);
             importFinishedMaterials(formData).then((res)=>{
                if (res.code == '0') {
                    this.$message({
                        message: res.msg,
                        type: 'success',
