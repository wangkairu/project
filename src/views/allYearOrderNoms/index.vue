...
<script>
import {selectYearNorm,selectNormGraph} from '@/api/allYear'
...
export default{
    ...
    methods:{
      ...
      async selectNormGraph(){
        const params={
          year:this.query.date.split('-')[0],
          month:this.query.date.split('-')[1],
        }
        const res = await selectNormGraph(params)
        if(res.code==='0'){
          let sum=0
          this.graphTask = res.data.map((v,ind)=>{
            if(ind<=9){
              return {value:v.mesNetName,name:v.normName}
            }else{
              sum+=v.mesNetName
            }
          })
        ...
        reset(){ this.selectYearNorm()},
        search(){ this.selectYearNorm()},
        selectBlur(){
            this.selectYearNorm()
        },
        async selectYearNorm(){
          let arr=[]
            const res = await selectYearNorm(this.form)
            if(res.code === '0'){
