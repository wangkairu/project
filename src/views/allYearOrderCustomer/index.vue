    ...
    <div class="table-filter">
      ...
      <div class="table">
          ...
          >
              ...
              <el-table-column v-for="(v,ind) in column" :key="v+ind" :label="ind+1>9?(ind+1)+'':'0'+(ind+1)">
...
<script>
import {selectYearByCustomerName,selectCustomerGraph} from '@/api/allYear'
...
export default{
  ...
  mounted(){
      this.selectYearByCustomerName()
      this.dateTypeChange()
      this.selectCustomerGraph()
  ...
  methods:{
    ...
    async selectCustomerGraph(){
      const params={
        year:this.query.date.split('-')[0],
        month:this.query.date.split('-')[1],
      }
      const res = await selectCustomerGraph(params)
      if(res.code==='0'){
        let sum=0
      ...
      reset(){ this.selectYearByCustomerName()},
      search(){ this.selectYearByCustomerName()},
      selectBlur(){
          this.selectYearByCustomerName()
      },
      async selectYearByCustomerName(){
        let arr=[]
          const res = await selectYearByCustomerName(this.form)
          if(res.code === '0'){
